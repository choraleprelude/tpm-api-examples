...
Based on example from: 
    https://github.com/tpm2-software/tpm2-tools/blob/master/tools/tpm2_nvread.c
     and referencing esapi_delete_persistent_key.c 

Read NV object from persistent handle 
1. First get persistent handle with Esys_TR_FromTPMPublic
2. Call Esys_NV_ReadPublic to get NV public data for size info
3. Call Esys_NV_Read to read NV object from persistent handle 
4. Output NV object
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        printf("Usage: esapi_nvread_persistent NVhandle NVauth output_path tcti (e.g.: esapi_nvread_persistent 0x1000005 authpassword outputNV.obj mssim)\n");
        return 1;
    }
